...
def decision_tree_demo_v2(max_depth, max_leaf_nodes, train_data, valedation_data, test_data):
    # Initialize Decision Tree classifier
    tree_classifier = DecisionTreeClassifier(max_depth=max_depth, max_leaf_nodes=max_leaf_nodes, random_state=42)

    # Train the Decision Tree on the training data
    tree_classifier.fit(train_data[:, 0:2], train_data[:, 2])
    prediction_list = np.array([])
    for data in [train_data, valedation_data, test_data]:
        # Make predictions on the test data
        pred = tree_classifier.predict(data[:, 0:2])
        # Compute the accuracy of the predictions
        accuracy = np.mean(pred == data[:, 2])
        prediction_list = np.append(prediction_list, accuracy)
    return tree_classifier, prediction_list
