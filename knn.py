...
class KNNClassifier:
    ...
    def predict(self, X):
        ...
        k = self.k
        distace, index = self.index.search(X, k)

        prediction_list = []

        for idx in index:
            unique, counts = np.unique(self.Y_train[idx], return_counts=True)
            prediction_list.append(unique[np.argmax(counts)])
